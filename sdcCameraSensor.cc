...
// gradient function, FADBAD++ wrapper
...
void sdcCameraSensor::OnUpdate() {
	...
	if (stopSigns.size() > 0) {
		sdcSensorData::stopSignFrames++;
	}
	else {
		sdcSensorData::stopSignFrames = 0;
	}
	*/
	...
	Mat contours, contoursInv, ROIgrey, ROIbinary;
	Canny(imageROI,contours,50,150);
	cvtColor(imageROI, ROIgrey, CV_BGR2GRAY);
	//threshold(ROIgrey,ROIbinary,128,255,THRESH_BINARY);
	GaussianBlur(ROIgrey,ROIgrey, Size(5,5), 0, 0);
	adaptiveThreshold(ROIgrey,ROIbinary, 255,CV_ADAPTIVE_THRESH_MEAN_C,CV_THRESH_BINARY,21, -3 );
	...
	while (it!=lines.end()) {
			...
			float theta= (*it)[1]; // second element is angle theta
				...
				if ((PI/2 + 0.1) < theta < (PI - 0.1) && theta > right_lane_marker[1]) {
					...
					//std::cout << "Right lane data updated! rho: " << rho << "| theta: " << theta << std::endl;
				}
	...
	double a, b, c, d, e, u, v, n, k, lane_midpoint, eps = 150.0;
	...
	for (std::vector<double>::const_iterator q = vec_of_i_vals.begin(); q != vec_of_i_vals.end(); q++) {
		...
		if(left_curve_points_bot.size() > 1) {
			for (int j = 0; j < left_curve_points_bot.size() - 1; j++) {
				...
				g =  ROIbinary.at<uchar>(xf,yf-240);
				double bin_lg = g/255.;
				//std::cout << g << std::endl;
				left_curve_magnitude += bin_lg;

			}
		}

		//double left_curve_magnitude_total = ((1.*left_curve_points_bot.size()) - left_curve_magnitude)/(1.*left_curve_points_bot.size());
		double left_curve_magnitude_total = left_curve_magnitude;//((1.*left_curve_points_bot.size()) - left_curve_magnitude);
	...
	for (std::vector<double>::const_iterator q = vec_of_i_vals.begin(); q != vec_of_i_vals.end(); q++) {
		...
		if(right_curve_points_bot.size() > 1) {
			for (int j = 0; j < right_curve_points_bot.size() - 1; j++) {
				...
				//std::cout << bin_rg;
				//std::cout << g << std::endl;
				curve_magnitude += bin_rg;

			}
		}

		//double curve_magnitude_total = ((1.*right_curve_points_bot.size()) - curve_magnitude)/(1.*right_curve_points_bot.size());
		double curve_magnitude_total = curve_magnitude;// ((1.*right_curve_points_bot.size()) - curve_magnitude);
...
///////// END LCF LANE DETECTION
	...
	imshow("Lane Detection", ROIbinary);
